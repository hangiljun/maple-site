...
export default function ReviewDetailPage() {
  ...
  const [newComment, setNewComment] = useState({ nickname: '', password: '', content: '' }); // 새 댓글 입력 상태
    ...
    <div style={{ backgroundColor: '#0F172A', minHeight: '100vh', color: '#F8FAFC', fontFamily: "'Noto Sans KR', sans-serif" }}>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '60px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          ...
          <button onClick={handleDeletePost} style={{ backgroundColor: 'transparent', border: '1px solid #ef4444', color: '#ef4444', padding: '5px 10px', borderRadius: '5px', fontSize: '12px', cursor: 'pointer' }}>삭제</button>
        </div>
        ...
        <div style={{ marginTop: '50px', borderTop: '1px solid #334155', paddingTop: '30px' }}>
            ...
            {comments.map((c) => (
              <div key={c.id} style={{ padding: '15px', borderBottom: '1px solid #1E293B', display: 'flex', justifyContent: 'space-between' }}>
                <div>
                  ...
                  <div style={{ fontSize: '15px', color: '#CBD5E1' }}>{c.content}</div>
                </div>
                <button onClick={() => handleDeleteComment(c.id, c.password)} style={{ background: 'none', border: 'none', color: '#64748B', cursor: 'pointer', fontSize: '12px' }}>삭제</button>
              </div>
            ))}
          </div>
          ...
          <form onSubmit={handleAddComment} style={{ backgroundColor: '#1E293B', padding: '20px', borderRadius: '10px' }}>
            ...
            <textarea placeholder="댓글 내용을 입력하세요" value={newComment.content} onChange={e => setNewComment({...newComment, content: e.target.value})} style={{ ...inputStyle, width: '100%', height: '80px', resize: 'none' }} />
            <button type="submit" style={{ width: '100%', marginTop: '10px', padding: '12px', backgroundColor: '#FF9000', color: '#000', border: 'none', borderRadius: '5px', fontWeight: 'bold', cursor: 'pointer' }}>댓글 등록</button>
          </form>
        </div>
...
const inputStyle = { backgroundColor: '#0F172A', border: '1px solid #334155', color: '#FFF', padding: '10px', borderRadius: '5px', outline: 'none', fontSize: '14px' };