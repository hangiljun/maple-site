    ...
    <div style={{ backgroundColor: '#F9F7F2', minHeight: '100vh', color: '#333', fontFamily: "'Noto Sans KR', sans-serif" }}>
      ...
      <div style={{ width: '100%', height: '350px', backgroundColor: '#DDD', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center', width: '100%', backgroundColor: 'rgba(255,255,255,0.8)', padding: '30px 0' }}>
          <h1 style={{ fontSize: '38px', fontWeight: '900', color: '#222', marginBottom: '10px' }}>메이플 아이템 비교해서 최고가 판매 하세요</h1>
      ...
      <div style={{ padding: '80px 60px', backgroundColor: '#F3F0E9' }}>
        <h2 style={{ textAlign: 'center', fontSize: '32px', marginBottom: '50px' }}><span style={{ color: '#FF9000' }}>메이플 아이템</span> 업체 비교, 무엇이 다를까요?</h2>
      ...
      <footer style={{ backgroundColor: '#FFF', padding: '40px', textAlign: 'center', color: '#999', fontSize: '12px', borderTop: '1px solid #E5E0D5' }}>© 2026 메이플 아이템. All rights reserved.</footer>
    </div>
  );
}
...
function ComparisonCard({ title, subtitle, items, isMain = false }: any) {
  return (
    <div style={{ backgroundColor: '#FFF', padding: '30px', borderRadius: '20px', width: '300px', border: isMain ? '2px solid #FF9000' : '1px solid #E5E0D5', boxShadow: isMain ? '0 10px 30px rgba(255,144,0,0.2)' : 'none', transform: isMain ? 'scale(1.05)' : 'none', position: 'relative', transition: '0.3s' }}>
      ...
      <ul style={{ listStyle: 'none', padding: 0, fontSize: '14px', lineHeight: '2.4' }}>
        {items.map((text: string, i: number) => (
          <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', marginBottom: '10px' }}>
            <span style={{ color: isMain ? '#FF9000' : '#CCC', fontWeight: 'bold' }}>✔</span><span style={{ flex: 1 }}>{text}</span>
          </li>
        ))}
      </ul>
    </div>
