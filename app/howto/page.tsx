    ...
    <div style={{ backgroundColor: '#0F172A', minHeight: '100vh', fontFamily: "'Noto Sans KR', sans-serif", color: '#F8FAFC' }}>
      ...
      <div style={{ padding: '60px 5%', maxWidth: '1000px', margin: '0 auto' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
          {howtos.length === 0 ? <div style={{textAlign:'center', color:'#666'}}>등록된 거래 방법이 없습니다.</div> : 
           howtos.map((h, index) => (
            <div key={h.id} style={{ display: 'flex', gap: '25px', backgroundColor: '#1E293B', padding: '35px', borderRadius: '20px', border: '1px solid #334155', alignItems: 'center' }}>
              <div style={{ width: '60px', height: '60px', backgroundColor: index === 0 ? '#FF9000' : '#334155', borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center', fontSize: '24px', fontWeight: '900', color: index === 0 ? '#000' : '#FFF', flexShrink: 0 }}>{index + 1}</div>
              <div style={{ flex: 1 }}>
